...
from faceapp.utils.processes.process_outputs import CleanupOutput
...
class LocalFileCleanup(Process):
    ...
    async def ainvoke(
        self, input_path: str, output_path: str, *args, **kwargs
    ) -> ProcessOutput:
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        if os.path.exists(input_path):
            shutil.move(input_path, output_path)
        return CleanupOutput(cleanup_status=True)
...
class S3Cleanup(Process):
    async def ainvoke(
        ...
        return CleanupOutput(cleanup_status=status)
